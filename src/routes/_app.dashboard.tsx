...
function Dashboard() {
  ...
  useRealtime((e) => {
    if (e.type === "device.state" && e.device_id) {
      setDevices((ds) =>
        ds.map((d) =>
          d.id === e.device_id
            ? { ...d, last_state: { ...d.last_state, ...e.state }, online: true }
            : d,
        ),
      );
  ...
  async function allOff() {
    await Promise.allSettled(
      devices
        ...
        .map((d) => api.post(`/devices/${d.id}/command`, { command: "turn_off" })),
    );
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      {(scenes.length > 0 || devices.some((d) => d.last_state?.power)) && (
        <section>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            ...
            {scenes.map((s) => (
              <button
                ...
                onClick={async () => {
                  try {
                    await api.post(`/scenes/${s.id}/activate`);
        ...
        .map(({ room, items }) => (
          <section key={room.id}>
            <div className="mb-4 flex items-center gap-2">
              ...
              />
              <h2 className="text-sm font-medium tracking-tight">{room.name}</h2>
      ...
      {devices.length === 0 && !error && (
        <div className="rounded-lg border border-dashed border-border bg-muted/30 p-10 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            The backend returned no devices. Check that the FastAPI service is running.
          </p>
        </div>
      )}
    </div>
